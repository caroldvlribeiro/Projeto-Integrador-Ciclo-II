...
{
    $sql = "SELECT o.id_orcamento, c.nm_cliente, c.cd_telefone, c.nm_endereco, o.dt_pedido, o.ds_descricao, o.vl_total, o.st_orcamento, v.id_venda,  v.dt_venda, p.vl_pagamento_entrada, p.vl_pagamento_saida, vd.nm_vendedor FROM orcamento o JOIN cliente c USING(cd_cliente) LEFT JOIN venda v USING(id_orcamento) LEFT JOIN pagamento p USING(id_orcamento) LEFT JOIN vendedor vd USING(id_vendedor)  WHERE 1=1";
    $params = [];
...
public function buscarPorNome(string $nm)
{
 ...
 public function filtrarPorPeriodo(string $ini, string $fim): array
    ...
    public function listarComFiltros(string $busca = '', string $dataInicio = '', string $dataFim = ''): array
    {
        ...
        if (!empty($busca)) {
            $sql .= " AND c.nm_cliente LIKE :busca";
            $params['busca'] = "%{$busca}%";
        }

        if (!empty($dataInicio) && !empty($dataFim)) {
            $sql .= " AND o.dt_pedido BETWEEN :dataInicio AND :dataFim";
